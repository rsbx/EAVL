...
void EAVLp_Node_print(EAVLp_node_t* eavl_node);
void EAVLp_Tree_print(char* prefix, char* end, char* prefix2, int dir, EAVLp_node_t* root);
EAVL_dir_t Node_CMP(void* ref_value, EAVLp_node_t* ref_node, EAVLp_node_t* node, void* data);
int Node_verify(EAVLp_node_t* eavl_node, EAVLp_node_t* childL, EAVLp_node_t* childR, void* data);
int Node_fixup(EAVLp_node_t* eavl_node, EAVLp_node_t* childL, EAVLp_node_t* childR, void* data);
void Init_nodes(struct node fnodes[], EAVLp_node_t* fnodep[], unsigned int count);
void check_reset(unsigned int count, unsigned int state);
int check_tree(EAVLp_context_t* context);
void check_op(EAVLp_context_t* context, unsigned int index, int (*op)(EAVLp_context_t* context, unsigned int k), int expect, char* operation, unsigned int state);
void init_tree_context(EAVLp_tree_t* tree, EAVLp_context_t* context);
void build_tree(EAVLp_tree_t* tree, EAVLp_context_t* context, unsigned int count);
int insert(EAVLp_context_t* context, unsigned int k);
int Eremove(EAVLp_context_t* context, unsigned int k);
int Nfixup(EAVLp_context_t* context, unsigned int k);
int traverse(EAVLp_context_t* context, EAVL_dir_t dir, EAVL_order_t order);
...
int check_tree(
	...
	{
	EAVLp_context_t		checkcontext;
	unsigned int		i;
	EAVLp_node_t*		dummy;
	int			error;
	int			result = 0;

	if ((error = EAVLp_Context_Init(&checkcontext, NULL)) != EAVL_OK)
		{
		printf("ERROR: Context_Init: %d\n", error);
		printf("\t%s:%u\n", __FILE__, __LINE__);
		exit(1);
		}
	if ((error = EAVLp_Context_Associate(&checkcontext, context->tree)) != EAVL_OK)
		{
...
out:
	if ((error = EAVLp_Context_Disassociate(&checkcontext)) != EAVL_OK)
		{
...
void check_op(
		EAVLp_context_t*	context,
		unsigned int		index,
		int (*op)(EAVLp_context_t* context, unsigned int k),
		int			expect,
		char*			operation,
		unsigned int		state
		)
...
void EAVLp_Tree_print(char*		prefix,
		char*			end,
		char*			prefix2,
		int			dir,
		EAVLp_node_t*		root
	...
	if (root && EAVLp_GET_CHILD(root, RIGHT))
		{
		if (dir != LEFT)
			{
			last = SAME;
		...
		EAVLp_Tree_print(prefix, nend, last, RIGHT, EAVLp_GET_CHILD(root, RIGHT));
		printf("%s%s%s\n", prefix, last, DIFF);
		}

	printf("%s%c---(", prefix, bend[dir]);
	EAVLp_Node_print(root);
	printf(")[a:%p  p:%p  L:%p  R:%p  b:%u]\n",
			(void*)root,
			(void*)EAVLp_GET_PARENT(root),
			(void*)EAVLp_GET_CHILD(root, 0),
			(void*)EAVLp_GET_CHILD(root, 1),
			EAVLp_GET_BAL(root)
			);

	if (root && EAVLp_GET_CHILD(root, LEFT))
		{
		if (dir != RIGHT)
			{
			last = SAME;
		...
		printf("%s%s%s\n", prefix, last, DIFF);
		EAVLp_Tree_print(prefix, nend, last, LEFT, EAVLp_GET_CHILD(root, LEFT));
		}
...
//	if ((error = EAVLp_Tree_Init(tree, NULL, &cbset)) != EAVL_OK)
//		{
...
//		}
//
//	if ((error = EAVLp_Context_Init(context, NULL, NULL)) != EAVL_OK)
//		{
//		printf("ERROR: Context_Init: %d\n", error);
//		printf("\t%s:%u\n", __FILE__, __LINE__);
//		exit(1);
//		}
//
//	if ((error = EAVLp_Context_Associate(context, tree)) != EAVL_OK)
//		{
//		printf("ERROR: Context_Associate: %d\n", error);
//		printf("\t%s:%u\n", __FILE__, __LINE__);
//		exit(1);
//		}
	...
	if ((error = EAVLp_Clear(context, &Node_release)) != EAVL_OK)
		{
...
void build_tree(
	...
	if ((error = EAVLp_Load(context, count, nodep)) != EAVL_OK)
		{
...
int insert(
	...
	{
	EAVLp_node_t*		existing;
	int			error;

//	printf("%d\n\n", k);

	error = EAVLp_Insert(context, nodep[k], &existing);
...
//	printf("%d\n\n", k);
	...
	error = EAVLp_Find(context, EAVL_FIND_EQ, NULL, NULL, nodep[k], &dummy);
	if (error != EAVL_OK)
		{
	...
	error = EAVLp_Remove(context, NULL);
...
int traverse(
	...
	error = EAVLp_First(context, dir, order, &result);
	...
	while (error == EAVL_OK)
		...
		error = EAVLp_Next(context, dir, order, &result);
		}
...
/*
	...
	INFO("EAVLp_tree_t:", EAVLp_tree_t);
	INFO("EAVLp_node_t:", EAVLp_node_t);
	INFO("EAVLp_context_t:", EAVLp_context_t);
	printf("\n");
*/

	EAVLp_Checks_Enabled = EAVLp_Checks_Available;
	...
	if ((error = EAVLp_Context_Init(&context, NULL)) != EAVL_OK)
		{
		printf("ERROR: Context_Init: %d\n", error);
		printf("\t%s:%u\n", __FILE__, __LINE__);
		exit(1);
	...
	*buffer = '\0';
	...
	printf("\n");
	EAVLp_Tree_print(buffer, buffer, "", 2, tree.root);
	printf("\n");


	for (k=EAVL_DIR_LEFT; k<=EAVL_DIR_RIGHT; k++)
		{
		EAVLp_node_t*		result = NULL;

		if (EAVL_OK != (error = EAVLp_First(
				&context,
				k,
				EAVL_ORDER_IN,
				&result
